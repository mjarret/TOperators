...
while [ $rsync_status -eq 30 ]; do
    # Use expect to handle rsync with a timeout
    expect -c "
    set timeout 15
    spawn rsync -v --exclude-from='exclude.txt' ./* ${USER}@${HOST}:${TARGET_DIR}/
    interact
    "
...
# Using expect to rename Makefile and run make and sbatch commands
expect -c "
spawn ssh -t ${USER}@${HOST}
expect \"*\$ \" 
send \"mv ${TARGET_DIR}/Makefile.cluster ${TARGET_DIR}/Makefile\r\"
expect \"*\$ \" 
send \"module load boost\r\"
expect \"*\$ \" 
send \"/usr/bin/make -C ${TARGET_DIR}\r\"
expect \"*\$ \" 
send \"sbatch ${TARGET_DIR}/runscript.slurm\r\"
expect \"*\$ \" 
send \"exit\r\"
interact
"
