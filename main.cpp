...
std::set<SO6> fileRead(int8_t tc, std::vector<SO6> tbase) {
    std::ifstream tfile;
    tfile.open(("data/T" + std::to_string(tc) + ".txt").c_str());
    if(!tfile) {
    ...
    std::set<SO6> tset;
    char hist;
    std::string mat; //Now unused except as a buffer
    long i = 0;
...
void writeResults(int8_t i, int8_t tsCount, int8_t currentCount, std::set<SO6> &next) {
    if(!saveResults) return;
    auto start = std::chrono::high_resolution_clock::now();
    std::string fileName = "data/T" + std::to_string(i+1) + ".tmp";
    std::ofstream write = std::ofstream(fileName);
    write << +tsCount << ' ' << +currentCount << '\n';
    for(SO6 n : next) write<<n;
    write.close();
    std::rename(("data/T" + std::to_string(i+1) + ".tmp").c_str(), ("data/T" + std::to_string(i+1) + ".txt").c_str());
    auto end = std::chrono::high_resolution_clock::now();
    auto ret = std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count();
    std::cout<<">>>Wrote T-Count "<<(i+1)<<" to 'data/T"<<(i+1)<<".txt' in " << ret << "ms\n";
}
int main(){
    // Z2 tmp = Z2(7,0,3);
    // Z2 tmp2 = Z2(1,0,1);
    ...
    std::set<SO6> prior;
    std::set<SO6> current({identity()});
    std::set<SO6> next;
    int start = 0;
    
    std::vector<SO6> tsv; //t count 1 matrices
    ...
    for(int8_t i = start; i<tCount; i++){
        ...
        // Main loop here
        std::ifstream tfile;
        tfile.open(("data/T" + std::to_string(i + 1) + ".txt").c_str());
        ...
        //     for(SO6 p : prior) next.erase(p); // Erase T-1
            ...
            std::string str;
            std::getline(tfile, str);
            std::stringstream s(str);
            std::getline(s, str, ' ');
            int8_t tsCount = std::stoi(str);
            std::getline(s, str, ' ');
            currentCount = std::stoi(str);
            tfile.close();
        ...
        // Begin reporting
        auto ret = std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count();
        std::cout << ">>>Found " << current.size()-prior.size() << " new matrices in " << ret << "ms\n";
        prior.clear();
    }
    std::chrono::duration<double> timeelapsed = std::chrono::high_resolution_clock::now() - tbefore;
    std::cout<< "\nTotal time elapsed: "<<std::chrono::duration_cast<std::chrono::milliseconds>(timeelapsed).count()<<"ms\n";
